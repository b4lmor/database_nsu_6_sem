...
| 3 | BEFORE INSERT OR UPDATE     | Проверяет, что отдел принадлежит зданию торговой точки.                      |
| 4 | BEFORE INSERT OR UPDATE     | Проверяет, что у поставщика достаточно товара для выполнения заказа.         |
| 5 | BEFORE INSERT OR UPDATE     | Проверяет, что торговая точка, привязанная к секции, является универмагом.   |
