...
## Переход от концептуальной к логической схеме БД
...
| reserved_by     | BIGINT    | REFERENCES product_order (id)                            |                            
| product_info_id | BIGINT    | NOT NULL, REFERENCES product_info (id) ON DELETE CASCADE |
...
### Доказательство соответствия 4ой нормальной форме
...
- Все связи "многие-ко-многим" реализованы через отдельные таблицы
...
| 7 | BEFORE UPDATE           | Устанавливает `reserved_by` у `vendor_id` при изменении статуса `product_order` на "**CONFIRMED**" |
